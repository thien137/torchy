...
class FunctionCtx:
    ...
    def engine(self):
        if self.saved_tensors:
            match self.saved_tensors[0].device:
                case "cpu":
                    import numpy as np 
                    return np 
                case "cuda":
            ...
            raise RuntimeError("No valid engine because function context is empty")
...
class BackwardFunction:
    ...
    def backward(self, grad_output):
        ...
        for inp, grad_inp in zip(self.ctx.saved_tensors, grad_inputs):
